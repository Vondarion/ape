...
/**
 * Deals with {@link ConstraintViolationException}s and maps the contained {@link ConstraintViolation}s to respective
 * {@link ValidationError}s stored within a {@link StatusMessage}.
 * 
 * @see StatusMessage
 */
public class ConstraintViolationMapper {
	...
	static StatusMessage		DEFAULT_MESSAGE	= new StatusMessage(400, "api.data_validation.error", null,
			"Data validation failed!");

	/**
	 * Converts the {@link ConstraintViolation}s contained within the specified {@link ConstraintViolationException} to
	 * respective {@link ValidationError}s.
	 * 
	 * @param constraint
	 *            The {@link ConstraintViolationException} to convert
	 * @param locale
	 *            The {@link Locale} used to get the message template/pattern
	 * @param resourceBundleName
	 *            The name of the {@link ResourceBundle} containing the I18N validation messages
	 * @return The respective {@link ValidationError}s
	 */
	public static ValidationError[] convertConstraintViolationExceptionToValidationErrors(
			final ConstraintViolationException constraint, final Locale locale, final String resourceBundleName) {
		Set<ConstraintViolation<?>> constraints = constraint.getConstraintViolations();
		log.info("convertConstraintViolationExceptionToValidationErrors({}, {}, {}", constraint, locale,
				resourceBundleName);
	 ...
	 * 
	 * @param constraints
	 *            The {@link ConstraintViolation}s to convert
	 * @param locale
	 *            The {@link Locale} used to get the message template/pattern
	 * @param resourceBundleName
	 *            The name of the {@link ResourceBundle} containing the I18N validation messages
	 * @return The respective {@link ValidationError}s
	 */
	public static ValidationError[] convertConstraintViolationsToValidationErrors(
			final Set<? extends ConstraintViolation<?>> constraints, final Locale locale,
			final String resourceBundleName) {
		log.info("convertConstraintViolationExceptionToValidationErrors({}, {}, {}", constraints, locale,
				resourceBundleName);
		...
		for ( final ConstraintViolation<?> violation : constraints ) {
			// get the message key
			final String messageKey = violation.getMessageTemplate().substring(1,
			...
			ValidationError error = new ValidationError(messageKey, violation.getMessage(), messageTemplate,
					violation.getPropertyPath().toString(), String.valueOf(violation.getInvalidValue()),
	 ...
	 * 
	 * @param violation
	 *            The {@link ConstraintViolation} from which to extract the attributes
	 * @param messageTemplate
	 *            The message template/pattern used for the message
	 * @return {@link Map} containing the name and value of all message parameter
	 */
	public static Map<String, String> extractMessageParameter(final ConstraintViolation<?> violation,
	...
	/**
	 * Extracts the names of all message placeholder (contained in curly braces) within the specified message template.
	 * 
	 * @param messageTemplate
	...
	public static StatusMessage getDefaultStatusMessage() {
		return new StatusMessage(DEFAULT_MESSAGE);
	}
